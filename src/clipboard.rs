...
mod tests {
    ...
    #[test]
    fn candidate_backends_prioritize_display_specific_tools() {
        let backends =
            candidate_backends_from_env(|name| matches!(name, "WAYLAND_DISPLAY" | "DISPLAY"));
    ...
    #[test]
    fn candidate_backends_include_tmux_when_present() {
        let backends = candidate_backends_from_env(|name| name == "TMUX");
        assert_eq!(
            backends,
            vec![
                ClipboardBackend::Pbcopy,
